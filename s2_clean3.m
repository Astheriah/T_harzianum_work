%Para procesar especificamente el modelo iYli21.mat y generar un archivo FASTA limpio y reducido basado en los genes del modelo
...
%% CONFIGURACIÓN – Procesar solo el modelo iYli21
modelo_mat = 'iYli21.mat';
...
%% 2. Leer GTF y construir mapeos protein_id -> gene_id, gene_id -> info
...
protein_to_geneid = containers.Map();   % protein_id -> gene_id
gene_info_map = containers.Map();       % gene_id -> struct
...
%% 4. Leer CSV de genes, transformar IDs para búsqueda (pero guardar originales)
fprintf('\n--- 4. Leyendo CSV de genes: %s ---\n', csvFile);
genesTable = readtable(csvFile, 'PreserveVariableNames', true);
fprintf('Columnas en el CSV: %s\n', strjoin(genesTable.Properties.VariableNames, ', '));
...
raw_ids = table2cell(genesTable(:, geneCol));
transformed_ids = cell(size(raw_ids));
for i = 1:length(raw_ids)
    id = char(raw_ids{i});
    new_id = regexprep(id, '^(YALI1)([A-Z])', '$1_$2');
    new_id = regexprep(new_id, '^(YALI0)([A-Z])', '$1_$2');
    transformed_ids{i} = new_id;
end
fprintf('Aplicando transformación YALI1X -> YALI1_X a la columna ''%s''...\n', geneCol);
fprintf('Ejemplos de transformación:\n');
for i = 1:min(5, length(transformed_ids))
    fprintf('  %s -> %s\n', raw_ids{i}, transformed_ids{i});
end

% Lista única de genes transformados (para búsqueda)
target_genes = unique(transformed_ids);
fprintf('Genes transformados a buscar: %d\n', length(target_genes));
if length(target_genes) > 0
    fprintf('Primeros 5 genes transformados: %s\n', strjoin(target_genes(1:min(5,end))', ', '));
end

% Mapeo de ID transformado -> ID original (para escribir en el FASTA final)
transformed_to_original = containers.Map();
for i = 1:length(transformed_ids)
    t = transformed_ids{i};
    o = raw_ids{i};
...
%% 5. Generar archivo _clean_1.faa con cabeceras = ID original del CSV (cuando corresponda)
fprintf('\n--- 5. Generando archivo con IDs originales del CSV -> %s_protein_clean_1.faa ---\n', baseName);
...
for i = 1:total_proteins
    ...
    if protein_to_geneid.isKey(prot_id)
        ...
        if ismember(gene_id, target_genes)
            % Usar el ID original del CSV (sin guión)
            ...
            newHeader = gene_id;   % ID transformado (con guión)
        end
    else
        newHeader = prot_id;   % fallback
    end
    fprintf(fid1, '>%s\n', newHeader);
    seq = seqs{i};
...
fprintf('Se reemplazaron %d cabeceras por IDs originales del CSV (de %d totales).\n', replaced_clean1, total_proteins);
...
for i = 1:total_proteins
    ...
    if protein_to_geneid.isKey(prot_id)
        gene_id = protein_to_geneid(prot_id);
        if ismember(gene_id, target_genes)
            % Esta proteína pertenece a un gen del CSV -> escribir en reducido
            % Cabecera: ID original del CSV (sin guión)
            if transformed_to_original.isKey(gene_id)
                newHeader = transformed_to_original(gene_id);
            else
                newHeader = gene_id;
...
fprintf('\n--- RESULTADOS EN GTF ---\n');
gtf_gene_ids = gene_info_map.keys;
found_in_gtf = {};
not_found_in_gtf = {};
for i = 1:length(target_genes)
    gene = target_genes{i};
    if ismember(gene, gtf_gene_ids)
        found_in_gtf{end+1} = gene;
    else
        not_found_in_gtf{end+1} = gene;
    end
end
...
if ~isempty(found_in_gtf)
    ...
    for k = 1:min(5, length(found_in_gtf))
        ...
        fprintf('  %s -> gene_name: ''%s'', locus_tag: ''%s'', product: ''%s'', protein_id: ''%s''\n', ...
            gene, info.gene_name, info.locus_tag, info.product, info.protein_id);
    end
end
...
for i = 1:length(found_in_gtf)
    gene = found_in_gtf{i};
    if gene_to_protein.isKey(gene)
        prot_id = gene_to_protein(gene);
        if faa_set.isKey(prot_id)
            found_in_faa{end+1} = gene;
        else
            not_found_in_faa{end+1} = gene;
        end
    else
        not_found_in_faa{end+1} = gene;
    end
...
total_csv = length(target_genes);
fprintf('Total genes en CSV: %d\n', total_csv);
fprintf('Encontrados en GTF: %d (%.1f%%)\n', length(found_in_gtf), length(found_in_gtf)/total_csv*100);
fprintf('Encontrados en FAA: %d (%.1f%%)\n', length(found_in_faa), length(found_in_faa)/total_csv*100);
